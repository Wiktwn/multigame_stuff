...
# the modifications i made seem to have messed with the nice little circles that are drawn to show where the peices are able to move to
# underlying gameplay seems to work fine, but the graphics are a bit bugged for some reason, maybe i forgot to add a global clause
...
def initialize():
    # didn't need to be defined because the game is currently unable to reset
    screen = pygame.display.set_mode((685, 685), pygame.RESIZABLE)
    pass

def start():
    global pressed, chessboard, is_blacks_turn, peice_rect_key, prev_board, can_move, need_to_check, white_win, black_win, insufficient_material_, stalemate
    global three_fold, promotion_sprites_dkt, current_peice_rects, square_rects, promotion_choice, should_i_print_him, all_possible_moves_to_make, clefted_board2, clefted_board, board_states, previous_board_state, chessboard
    global chess_sprites,chessboard_image,queshun,queshun,queshun_rect,chessboard_image,special_image,font,little_font,winsurface,white_win,black_win,insufficient_material_,stalemate,three_fold,need_to_check,screen,queshun
    global FRAMERATE,time_to_check,pressed,should_i_print_him,is_blacks_turn,in_testing,can_move,promotion_sprites_dkt,promotion_list,promotion_choice,game_name,g_rect,peices,colours,prev_board,proper_board,default
    global peice_rect_key,queens_dkt,board_states,last_peice,x_index_list,y_index_list,x_index_list,blk_start_row,castling_poses_b,castling_poses_w,wht_start_row,player_king_moved,bot_king_moved,menu_button,menu_lettres 
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:  # If the player clicks the mouse
                ...
                if pygame.Rect.collidepoint(menu_button, mouse_pos):
                    return # hand back control to the multigame
